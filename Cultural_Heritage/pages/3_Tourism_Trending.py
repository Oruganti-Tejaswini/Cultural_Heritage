...
# =================== TRENDS OVER YEARS ===================
    ...
    for idx, state in enumerate(state_selection):
        ...
        if forecast_enable and len(state_df) >= 2:
            ...
            future_years = np.array([state_df["Year_int"].max() + i for i in range(1, 7)]).reshape(-1, 1)
    ...
    if explorer_option == "Table View":
        ...
        with col1:
            ...
            if forecast_enable and len(drill_df) >= 2:
                ...
                model = LinearRegression().fit(X, y)
                future_years = np.array([drill_df["Year_int"].max() + i for i in range(1, 7)]).reshape(-1, 1)
                future_preds = model.predict(future_years)
